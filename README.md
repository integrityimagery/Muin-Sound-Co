...
The sequence, on every internal navigation:

1. If the header is collapsed, it grows back to full height (420ms).
2. The outgoing page comes apart into dust and blows away (340ms).
3. The new page fades in and settles up 22px (340ms, starting as the dust ends).
4. A vine draws itself in under the nav item you chose (520ms, from 300ms).
...
**Neither header snapshot cross-fades.** The browser's default dissolves both
— in `plus-lighter`, no less — and because the collapsed header's nav sits at
a different height from the expanded one's, that put two navigation bars on
screen at once, each at half brightness, for the length of the fade. It read as
a glitch because it is one. The incoming header is opaque from the first frame
instead, and `object-fit: cover` crops rather than scales it: the box is
already full width, so covering it never resizes the image, and a box growing
63px → 251px therefore *reveals* the expanded header downward at its true size.
That is the thing that reads as the header opening back out. Without the
`object-fit`, the default stretches a 63px snapshot to 251px and the lettering
smears the whole way.

**The dust is a mask, not particles.** `::view-transition-old(root)` is masked
with fractal noise generated by an inline SVG filter, and the *noise* scales up
as the page fades — so different parts of it thin out at different moments and
the page comes apart into grain rather than dimming evenly, with a small drift
right to give the dust somewhere to go. Nothing to ship, nothing to keep in
step with the palettes. `npm run audit` greps the built CSS for `feTurbulence`,
because a data URI that a minifier re-escapes badly is not an error — the mask
just silently masks nothing and you get a plain fade back.
...
every test, and was in practice invisible.
...
Where you are is marked by a weight change and **a vine drawn under the label**,
which grows in on arrival. It replaces two older markers at once: the green leaf
glyph that sat before the current page's label, and the flat rule that marked
the current section. One marker, one idea.

It is a stroked path stretched to whatever the label is wide, which is why it
is a stroke and nothing else — `vector-effect` keeps the line an even weight at
any width, where a leaf or a curl would be squashed along with the box. It is
absolutely positioned so it cannot alter the link's box: a block-level marker
here once made story pages' header 3px taller than everywhere else, which the
header-parity test caught.

Never colour alone (WCAG 1.4.1): weight 700 plus the vine. `--vine-stem` is
already enforced at 3:1 against every page ground by `npm run contrast`, and
`npm run test:a11y` asserts the vine is present *and finishes drawing* — a
marker stuck half-grown is not a marker.
